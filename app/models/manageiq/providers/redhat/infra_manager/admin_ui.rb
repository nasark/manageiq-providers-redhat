module ManageIQ::Providers::Redhat::InfraManager::AdminUI
  ...
  def queue_generate_admin_ui_url
    task_opts = {
      :action => 'Generate Redhat Admin UI URL for EMS'
    }
  ...
  def generate_admin_ui_url
    $rhevm_log.info("Generating Redhat Admin UI URL for EMS with identifier '#{id}'.")
    ...
    request.headers = {
      ...
      'User-Agent'   => "manageiq-providers-rhv/#{ManageIQ::Providers::Redhat::VERSION}"
    }
